...
Example: python benchmark.py --method ChatGPT-STK --dataset Essay --detectLLM ChatGPT-turbo --conf_threshold 0.01 --filter_threshold 0.2 --sentence_num 3 --load_path Essay-ChatGPT-turbo (Note that the load_path is Essay-ChatGPT-turbo, which means you need to train in Essay's ChatGPT-turbo under the same settings first.)
